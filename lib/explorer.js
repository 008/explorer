...
module.exports = {
  ...
  exec_blockhash: function(height, cb) {
    ...
    exec(cmd, {maxBuffer: 5024 * 1024 * 4}, function (error, stdout, stderr) { //beet
     if (error) {
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') { 
        ...
        module.exports.is_unique(arr_vout, vout[i].scriptPubKey.address, function(unique, index) {
          if (unique == true) {
            // unique vout
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              arr_vout.push({address: vout[i].scriptPubKey.address, amount: amount_sat});
              loop.next();
            });
          } else {
            // already exists
      ...
      if (vout[0].scriptPubKey.type == 'nonstandard') {
        if ( arr_vin.length > 0 && arr_vout.length > 0 ) {
          if (arr_vin[0].address == arr_vout[0].address) {
            //PoS
  ...
  get_input_address: function(input, vout, cb) {
    var address = [];
    if (input.coinbase) {
      var amount = 0;
      module.exports.syncLoop(vout.length, function (loop) {
        ...
        address.push({hash: 'coinbase', amount: amount});
        return cb(address);
      });
    } else {
          ...
          module.exports.syncLoop(tx.vout.length, function (loop) {
            ...
            if (tx.vout[i].n == input.vout) {
              ...
              if (tx.vout[i].scriptPubKey.address) {
                address.push({hash: tx.vout[i].scriptPubKey.address, amount:tx.vout[i].value});  
              }
                loop.break(true);
                loop.next();
              //});
  ...
  prepare_vin: function(tx, cb) {
    ...
    module.exports.syncLoop(tx.vin.length, function (loop) {
      var i = loop.iteration();
      module.exports.get_input_address(tx.vin[i], tx.vout, function(address){
        if (address && address.length) {
          //console.log('vin');
          module.exports.is_unique(arr_vin, address[0].hash, function(unique, index) {
            if (unique == true) {
              module.exports.convert_to_satoshi(parseFloat(address[0].amount), function(amount_sat){
                arr_vin.push({address:address[0].hash, amount:amount_sat});
                loop.next();
              });
            } else {
              module.exports.convert_to_satoshi(parseFloat(address[0].amount), function(amount_sat){
